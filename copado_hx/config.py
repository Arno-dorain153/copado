...
def encrypt_token(token: Optional[str]) -> Optional[str]:
    if not token:
        return None
    # XOR cipher with a key to prevent plaintext tokens in db.json
    key = 0x5F
    cipher = "".join(chr(ord(c) ^ key) for c in token)
...
def decrypt_token(encrypted_token: Optional[str]) -> Optional[str]:
    if not encrypted_token:
        return None
    try:
        cipher = base64.b64decode(encrypted_token.encode('utf-8')).decode('utf-8')
        key = 0x5F
...
def get_auth() -> Dict[str, Any]:
    """Returns authentication details, decrypting the token if present."""
    db = load_db()
    auth = db.get("auth", DEFAULT_DB["auth"]).copy()
    if auth.get("token"):
        auth["token"] = decrypt_token(auth["token"])
    return auth

def save_auth(username: Optional[str], token: Optional[str], org_id: Optional[str] = None, environment: Optional[str] = None):
    """Saves authentication details with an encrypted token."""
    db = load_db()
    db["auth"] = {
        "username": username,
